...
def graphs():
    ...
    # Page visites
    page_visit = df['page'].apply(
        lambda x :'/'+ x.split('/',maxsplit=3)[-1]
        ).value_counts().reset_index()
    fig = px.bar(page_visit,
        x='page',
        y='index',
        color="index",
        labels={
    ...
    fig = px.bar_polar(city_df,
        r='city',
        theta='index',
        log_r=True,color='city',
    ...
    total_visitors["repeated"] = total_visitors.apply(lambda x: x.ip - x["unique"], axis=1)
    ...
    fig = px.bar(hourly,x='time',y='page',
        labels={
